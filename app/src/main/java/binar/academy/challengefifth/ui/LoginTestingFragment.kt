...
class LoginTestingFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.registershowTextView.setOnClickListener {
            findNavController().navigate(LoginTestingFragmentDirections.actionLoginTestingFragmentToRegisterFragment())
        }
    ...
    fun navigationUi(){
        viewModel.getValidation().observe(viewLifecycleOwner){ nav ->
            if(nav== 1){
                findNavController().navigate(LoginTestingFragmentDirections.actionLoginTestingFragmentToHomeFragment())
